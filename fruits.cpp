...
int main()
{
	...
	while(1)
	{
		...
		for ( int i = 0 ; i < m ; i++ )
		{
			for ( int j = 0 ; j < n ; j++ )
			{
				if ( (img.at<Vec3b>(i,j)[2] <= thrmax && img.at<Vec3b>(i,j)[2] >= thrmin) && ( img.at<Vec3b>(i,j)[1] <= thbmax && img.at<Vec3b>(i,j)[1] >= thbmin ) && ( img.at<Vec3b>(i,j)[0] <= thgmax && img.at<Vec3b>(i,j)[0] >= thgmin ) )
				{
					for ( int k = 0 ; k < 3 ; k++ )
					{
